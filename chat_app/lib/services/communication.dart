...
class Communication {
  Communication({this.port = 8080, required this.onUpdate});
    ...
    return interfaces
        .where((e) =>
            e.addresses.first.address
                .indexOf(interfaces.first.addresses.first.address) ==
            0)
        .first
        .addresses
        .first
...
print(senderName);
    if (msg != null) {
      onUpdate(Message(from, msg, senderId ?? '', senderName ?? ''));
    }
  }
  ...
  void sendMessage(String msg, String senderId, String senderName) async {
    ...
    for (var i = 1; i < 255; i++) {
      print('$threeOctet.$i');
      _sendRequest('$threeOctet.$i',
          "?ip=$ip&msg=$msg&senderId=$senderId&senderName=$senderName");
    }
  }
